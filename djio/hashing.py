...
def int_to_bytes(i: int,
    ...
    i_abs = i if i >=0 else int(math.fabs(i))
    ...
    for idx in range(0, width):
        ...
        bsis[idx] = (i_abs & _mask) >> (shift * 8)
    # If the original integer (i) was negative we'll flip on the highest-order bit (of the highest-order byte, which is
    # to say the one at index 0), otherwise we'll flip it off.
...
def djiohash_v1(geometry_type_code: int,
    ...
    b8_128 = [0] * 120 #TODO: 120 is a magic constant!!!
    ...
    for coord in coordinates:
        coord_bsis = _coord_to_bytes(coord, width=how_many_bytes_per_coord, precision=precision)
    ...
    bytes = bytearray([b1] + b2_4 + b5_7 + b8_128)
